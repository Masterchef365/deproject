...
use glow::{HasContext, Context, NativeBuffer, VertexArray, Program};
...
impl FloorProgram {
    pub fn new(gl: &Context, model: ProjectorModel, tracking_rx: Receiver<BlobTrackerDelta>, plane: Plane) -> Result<Self> {
        unsafe {
    ...
    fn frame(&mut self, gl: &Context) {
        ...
        for v in &mut self.line_verts {
            v.pos = *self.plane.from_planespace(Point3::from(v.pos).xzy()).coords.as_ref();
        }
